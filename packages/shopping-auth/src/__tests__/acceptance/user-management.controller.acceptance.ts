...
describe('UserManagementController', () => {
  ...
  const controller: UserManagementController | undefined = undefined;
  ...
  before(async () => {
    const roleRepo = await app.get<RoleRepository>(
      'repositories.RoleRepository',
    );
    const customerRole = await roleRepo.findOne({where: {name: 'customer'}});
    userData.roleId = customerRole?.id || '';
  });
  let adminToken: string;
  before(async () => {
    const res = await client
      .post('/users/login')
      .send({email: 'admin@example.com', password: 'admin12345678'})
      .expect(200);
  ...
  describe('authentication', () => {
    ...
    it('users/me returns the current user profile when a valid JWT token is provided', async () => {
      ...
      expect(userProfile.roleId).to.equal(newUser.roleId);
    });
  ...
  async function createAUser() {
    ...
    const role = await roleRepo.findOne({where: {name: 'customer'}});
  ...
  async function givenAnExpiredToken() {
    const newUser = await createAUser();
    const newUserWithRole = await userRepo.findById(newUser.id, {
      include: [{relation: 'role'}],
    });
    const jwtPrivateKey = app.getSync<string>('jwt.private.key');
    const jwtPublicKey = app.getSync<string>('jwt.public.key');
    const jwtIssuer = app.getSync<string>('jwt.issuer');
    ...
    const userService = app.getSync<UserManagementService>(
      UserServiceBindings.USER_SERVICE,
    );
    const userProfile = userService.convertToUserProfile(newUserWithRole);
